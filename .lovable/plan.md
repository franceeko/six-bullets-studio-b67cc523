...
- **Para de funcionar depois de um tempo.** Hoje o watchdog de FPS baixa o nível de qualidade e, em sequência, pode chegar ao piso; e quando o contexto WebGL cai (`webglcontextlost`) o canvas some e nunca volta, porque o código só remonta "no próximo mount". Vou colocar um piso de qualidade (nunca abaixo de `eco`), e no `webglcontextrestored` recriar shader, programa e uniforms de verdade em vez de só reexibir o canvas. Também recomeço a animação ao voltar o foco/aba mesmo se o pause tiver empilhado.
